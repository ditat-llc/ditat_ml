...
class Persona:
	...
	def _internal_process(
		...
		if re.search(r'^c[efiotc]o$', last_word):
			...
			temp_function = None
			for persona, synonym in type(self).PERSONA_SYNONYMS.items():
		...
		# II. "Function seniority". 2-word combination like "Finance Manager" (Function Seniority)
			...
			for persona, synonym in type(self).PERSONA_SYNONYMS.items():
				if first in synonym:
					temp_function = persona
					break
			temp_seniority = type(self).SENIORITY_MAPPING.get(second)
		...
		# III. Same logic as II, but instead of Finance manager, it is Manager of finance.
			...
			for persona, synonym in type(self).PERSONA_SYNONYMS.items():
				if third in synonym:
					temp_function = persona
					break
			temp_seniority = type(self).SENIORITY_MAPPING.get(first)
			if temp_function and temp_seniority:
				return {'seniority': temp_seniority, 'function': temp_function}
		...
		# it this does not happen we assume the single word references a Function with 'default_' seniority.
			...
			temp_function = None
			temp_seniority = None
			if value in type(self).SENIORITY_MAPPING:
				temp_seniority = type(self).SENIORITY_MAPPING[value]
				...
				for persona, synonym in type(self).PERSONA_SYNONYMS.items():
					if value in synonym:
						temp_function = persona
						break
				temp_seniority = type(self).SENIORITY_MAPPING['default_']			
			if temp_function and temp_seniority:
				return {'seniority': temp_seniority, 'function': temp_function}
		...
		if temp_function:
			for s in type(self).SENIORITY_MAPPING:
				if s in value.split():
					temp_seniority = type(self).SENIORITY_MAPPING[s]
